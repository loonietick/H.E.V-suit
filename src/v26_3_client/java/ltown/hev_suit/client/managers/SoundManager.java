...
public class SoundManager {
    ...
    private static void registerSound(String name) {
        try {
            Identifier soundId = Identifier.fromNamespaceAndPath("hev_suit", name);
            SoundEvent sound = SoundEvent.createVariableRangeEvent(soundId);
            Registry.register(BuiltInRegistries.SOUND_EVENT, soundId, sound);
    ...
    public static void processSoundQueue(Minecraft client) {
    ...
    private static void maintainLoopingSounds(Minecraft client) {
        if (client == null || client.getSoundManager() == null) {
            return;
        }
    ...
    private static void processQueueChannel(Minecraft client, QueueChannel channel) {
    ...
    private static void playNextSound(Minecraft client, QueueChannel channel) {
        if (alertsSuppressed()) {
    ...
    private static void ensureGeigerLoop(Minecraft client) {
        if (!geigerLoopRequested || alertsSuppressed()) return;
        if (geigerSoundInstance != null && client.getSoundManager().isActive(geigerSoundInstance)) {
            return;
        }
    ...
    public static void playFlatline() {
        ...
        if (flatlineSoundInstance != null && client.getSoundManager().isActive(flatlineSoundInstance)) {
            return;
        }
    ...
    private static boolean playSoundInstance(Minecraft client, SoundInstance instance) {
        ...
        try {
            ...
            LOGGER.debug("Sound {} did not start immediately (result: {}), attempting fallback.", instance.getIdentifier(), result);
        ...
        // play(instance, seed) removed in MC 26.x
    ...
    private static boolean isChannelActive(Minecraft client, QueueChannel channel) {
        if (channel.currentSound == null) {
            return false;
        }
        if (channel.startupCooldown > 0) {
        ...
        return client.getSoundManager().isActive(channel.currentSound);
    }

    private static void interruptChannel(Minecraft client, QueueChannel channel) {
    ...
    private static Method resolvePlayMethod(boolean requireSeed) {
        Class<?> soundManagerClass = net.minecraft.client.sounds.SoundManager.class;
