...
public class SoundManager {
    ...
    static {
        // On 1.20.1, SoundManager#play(SoundInstance, int) is NOT a seeded-pitch overload -- the
        // int is a tick delay (SoundSystem stores it as startTicks.put(instance, ticks + delay)),
        // so a random int there schedules playback anywhere from "immediately" to "practically
        // never". Prefer the single-arg play(SoundInstance), which starts the sound synchronously
        // with no delay; only fall back to the delayed overload (with delay 0, not a random value)
        // if the immediate one genuinely isn't present.
        Method basic = resolvePlayMethod(false);
        Method seeded = resolvePlayMethod(true);
        SOUND_MANAGER_PLAY_METHOD = basic != null ? basic : seeded;
        SOUND_MANAGER_PLAY_WITH_SEED = basic == null && seeded != null;
    ...
    private static boolean playSoundInstance(MinecraftClient client, SoundInstance instance) {
        ...
        // 1.20.1's SoundManager#play(SoundInstance) returns void (no PlayResult on this version),
        // so go straight to the reflective lookup below, which resolves whatever play signature
        // this version actually has.
        if (SOUND_MANAGER_PLAY_METHOD != null) {
            try {
                if (SOUND_MANAGER_PLAY_WITH_SEED) {
                    // The int here is a tick delay, not a seed -- 0 means "start now".
                    SOUND_MANAGER_PLAY_METHOD.invoke(client.getSoundManager(), instance, 0);
                } else {
        ...
        if (!succeeded) {
            try {
                // Again: 0 = start now, not a seed.
                client.getSoundManager().play(instance, 0);
                succeeded = true;
            } catch (NoSuchMethodError ignored) {
                // Older signatures do not include the delay parameter.
            } catch (Throwable throwable) {
