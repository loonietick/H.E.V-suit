...
public class HudManager {
    ...
    private static void sprDrawGeneric(Object g, Identifier texId, int x, int y, int destW, int destH, Rect prc, int color) {
        ...
        m.scale(sx, sy, 1.0F);
        int tinted = withOpacity(color, HUD_OPACITY);
        // 1.20.1's DrawContext.drawTexture doesn't route through a blend-baked RenderPipeline
        // like 1.21.8+ does (RenderPipelines.GUI_TEXTURED) -- it relies on blend already being
        // enabled by the caller. Without this, our alpha tint (and cropped icons' soft edges)
        // gets written opaquely instead of blended, i.e. icons render fully solid/bold.
        RenderSystem.enableBlend();
        RenderSystem.defaultBlendFunc();
        RenderSystem.setShaderColor(((tinted >> 16) & 0xFF) / 255F, ((tinted >> 8) & 0xFF) / 255F, (tinted & 0xFF) / 255F, ((tinted >> 24) & 0xFF) / 255F);
        ((DrawContext) g).drawTexture(texId, 0, 0, (float)u, (float)v, rw, rh, fullW, fullH);
    ...
    private static void drawCroppedImageScaled(Object g, Identifier id, int x, int y, float scale, int color, Rect prc, int destW, int destH) {
        var m = ((DrawContext) g).getMatrices();
        m.push();
        m.scale(scale, scale, 1.0F);
     ...
     */
    private static int drawArmorGauge(Object g, int xLeft, int baselineY, float scale, int color, int armorPctClamped) {
    ...
    private static int drawFlashlightGauge(Object g, int xLeft, int baselineY, float scale, int color, int battery, boolean isOn) {
