...
public class CommandManager {
    ...
    public static void registerToggleCommands() {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            LiteralArgumentBuilder<FabricClientCommandSource> root = ClientCommands.literal("hev");
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildConfigCommand() {
        return ClientCommands.literal("config")
                .executes(context -> {
                    ...
                    if (client != null) {
                        client.execute(() -> {
                            if (client.gui.screen() instanceof HevSuitConfigScreen configScreen) {
                                configScreen.onClose();
                            } else {
                                client.setScreenAndShow(new HevSuitConfigScreen(client.gui.screen()));
                            }
                        });
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildToggleCommands() {
        ...
        toggle.then(ClientCommands.literal("all").executes(CommandManager::toggleAllFeatures));
        return toggle;
    }

    private static LiteralArgumentBuilder<FabricClientCommandSource> buildMainToggleSection() {
        LiteralArgumentBuilder<FabricClientCommandSource> main = ClientCommands.literal("main");
        main.then(ClientCommands.literal("enabled")
                .executes(ctx -> toggleOption(ctx, "HEV Suit", () -> SettingsManager.hevSuitEnabled, value -> SettingsManager.hevSuitEnabled = value)));
        main.then(ClientCommands.literal("pvp").executes(CommandManager::togglePvpMode));
        main.then(ClientCommands.literal("captions").executes(CommandManager::toggleCaptions));
        return main;
    }
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildHudToggleSection() {
        ...
        hud.then(ClientCommands.literal("threat_indicators")
        ...
        hud.then(ClientCommands.literal("align")
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildAudibleToggleSection() {
        LiteralArgumentBuilder<FabricClientCommandSource> audible = ClientCommands.literal("audible");
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildAudibleGeneralSection() {
        LiteralArgumentBuilder<FabricClientCommandSource> general = ClientCommands.literal("general");
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildAudibleHealthSection() {
        LiteralArgumentBuilder<FabricClientCommandSource> health = ClientCommands.literal("health");
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildAudibleWeaponSection() {
        LiteralArgumentBuilder<FabricClientCommandSource> weapons = ClientCommands.literal("weapons");
    ...
    private static void addToggleCommands(LiteralArgumentBuilder<FabricClientCommandSource> parent, ToggleDescriptor... toggles) {
        for (ToggleDescriptor toggle : toggles) {
            parent.then(ClientCommands.literal(toggle.command())
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildEditCommands() {
        LiteralArgumentBuilder<FabricClientCommandSource> edit = ClientCommands.literal("edit");
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildHudColorCommands() {
        LiteralArgumentBuilder<FabricClientCommandSource> hud = ClientCommands.literal("hud");
        LiteralArgumentBuilder<FabricClientCommandSource> color = ClientCommands.literal("color");
        color.then(ClientCommands.argument("hexcolor", StringArgumentType.word())
                .executes(context -> setBothHudColors(context, StringArgumentType.getString(context, "hexcolor"))));
        color.then(ClientCommands.literal("reset").executes(CommandManager::resetHudColors));
        color.then(ClientCommands.literal("primary")
                .then(ClientCommands.argument("hexcolor", StringArgumentType.word())
                        .executes(context -> setPrimaryColor(context, StringArgumentType.getString(context, "hexcolor")))));
        color.then(ClientCommands.literal("secondary")
                .then(ClientCommands.argument("hexcolor", StringArgumentType.word())
                        .executes(context -> setSecondaryColor(context, StringArgumentType.getString(context, "hexcolor")))));
        color.then(ClientCommands.literal("setall")
                .then(ClientCommands.argument("hexcolor", StringArgumentType.word())
                        .executes(context -> setBothHudColors(context, StringArgumentType.getString(context, "hexcolor")))));
        hud.then(color);
        return hud;
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildListQueueCommand() {
        return ClientCommands.literal("listqueue").executes(context -> {
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildClearQueueCommand() {
        return ClientCommands.literal("clearqueue").executes(context -> {
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> buildQueueSoundCommand() {
        return ClientCommands.literal("queuesound")
    ...
    private static int toggleHudEnabled(CommandContext<FabricClientCommandSource> context) {
        ...
        if (newValue) {
            context.getSource().sendFeedback(
                    Component.literal("Notice: Enable the resource pack that hides vanilla hearts/armor if desired.")
                            .setStyle(Style.EMPTY.withColor(ChatFormatting.RED)));
        } else {
            context.getSource().sendFeedback(
                    Component.literal("Notice: Disable the resource pack hiding vanilla hearts/armor.")
                            .setStyle(Style.EMPTY.withColor(ChatFormatting.RED)));
        }
    ...
    private static void sendInvalidColor(CommandContext<FabricClientCommandSource> context) {
        context.getSource().sendFeedback(
                Component.literal("Invalid hex color. Use format: #RRGGBB or RRGGBB")
                        .setStyle(Style.EMPTY.withColor(ChatFormatting.RED)));
    }

    private static Component colorPreview(String label, int color) {
        return Component.literal(label + ": ")
                .append(Component.literal("■■■").setStyle(Style.EMPTY.withColor(color & 0xFFFFFF)));
    }

    private static void sendPrimaryFeedback(FabricClientCommandSource source, String message) {
        source.sendFeedback(Component.literal(message).setStyle(Style.EMPTY.withColor(SettingsManager.hudPrimaryColor & 0xFFFFFF)));
    }
