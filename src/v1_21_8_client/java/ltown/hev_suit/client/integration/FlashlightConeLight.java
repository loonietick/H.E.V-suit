...
class FlashlightConeLight implements DynamicLightBehavior {
    ...
    public boolean hasChanged() {
        if (Math.abs(entity.getX() - prevX) >= 0.1
                ...
                || Math.abs(entity.getYaw() - prevYaw) >= 0.1
                || Math.abs(entity.getPitch() - prevPitch) >= 0.1) {
            prevX = entity.getX();
            prevY = entity.getY();
            prevZ = entity.getZ();
            prevYaw = entity.getYaw();
            prevPitch = entity.getPitch();
            computeMatrices();
            return true;
        }
        return false;
    ...
    private void computeMatrices() {
        ...
        matrix.rotate(RotationAxis.POSITIVE_Z.rotationDegrees(entity.getPitch()));
        matrix.rotate(RotationAxis.NEGATIVE_Z.rotation(MathHelper.HALF_PI));
        matrix.rotate(RotationAxis.POSITIVE_Y.rotationDegrees(entity.getYaw()));
        matrix.rotate(RotationAxis.POSITIVE_Y.rotation(MathHelper.HALF_PI));
