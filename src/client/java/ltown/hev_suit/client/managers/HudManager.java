...
import ltown.hev_suit.client.util.DrawCompat;
...
public class HudManager {
    ...
    private static void sprDrawGeneric(Object g, Identifier texId, int x, int y, int destW, int destH, Rect prc, int color) {
        ...
        var m = ltown.hev_suit.client.util.GfxCompat.matrices(g);
        ltown.hev_suit.client.util.MatrixCompat.push(m);
        float sx = outW / (float)rw;
        float sy = outH / (float)rh;
        ltown.hev_suit.client.util.MatrixCompat.translate(m, (float) x, (float) y);
        ltown.hev_suit.client.util.MatrixCompat.scale(m, sx, sy);
        ltown.hev_suit.client.util.DrawCompat.drawTexture(g, texId, 0, 0, (float)u, (float)v, rw, rh, fullW, fullH, withOpacity(color, HUD_OPACITY));
        ltown.hev_suit.client.util.MatrixCompat.pop(m);
    }
    ...
    private static Identifier tex(String name) {
        return ltown.hev_suit.client.util.IdCompat.of(MODID, "textures/gui/hud/" + name + ".png");
    }
    ...
    private static void drawImageScaled(Object g, Identifier id, int x, int y, float scale, int color) {
        ...
        var m = ltown.hev_suit.client.util.GfxCompat.matrices(g);
        ltown.hev_suit.client.util.MatrixCompat.push(m);
        ltown.hev_suit.client.util.MatrixCompat.scale(m, scale, scale);
        ...
        HudRenderCallback.EVENT.register((graphics, tickDelta) -> {
            ...
            if (SettingsManager.hudArmorEnabled) {
                ...
                if (SettingsManager.hudHealthEnabled) {
                    ...
                    ltown.hev_suit.client.util.GfxCompat.fill(graphics, barX1, barTop, barX2, barBot, withOpacity(PRIMARY, HUD_OPACITY));
    ...
    private static void fillTriangle(Object graphics, int[] xPoints, int[] yPoints, int color) {
        ...
        for (int y = minY; y <= maxY; y++) {
            ...
            if (intersections.size() >= 2) {
                ...
                ltown.hev_suit.client.util.GfxCompat.fill(graphics, x1, y, x2, y + 1, color);
            }
        }
    }
